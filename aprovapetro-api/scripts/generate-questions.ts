...
// Função que simularia a chamada para a API da OpenAI ou Google Gemini
async function callAIGenerator(topicName: string, subjectName: string, amount: number): Promise<any[]> {
  console.log(`[AI] Gerando ${amount} questões inéditas para o tópico: ${topicName} (${subjectName})...`);
  
  // Simulando delay da rede da IA (2 segundos)
...
async function main() {
  ...
  for (const topic of topics) {
    // Para o MVP, vamos gerar 5 questões por tópico para popular o banco.
    // Numa versão de produção, isso rodaria em background gerando centenas.
    const questions = await callAIGenerator(topic.name, topic.subject.name, 5);
    
    for (const q of questions) {
      // Ajustando a opção correta gerada aleatoriamente
      const optionsData = q.options.map((text: string, idx: number) => ({
        text,
        orderIndex: idx
      }));
